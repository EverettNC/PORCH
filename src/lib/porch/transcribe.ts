...
function durationMsFrom(body: unknown): number {
  ...
  if (typeof rec.durationMs === "number" && Number.isFinite(rec.durationMs)) {
    return Math.max(0, Math.round(rec.durationMs));
  }
...
function earFor(source: TranscribeInput["source"], seated: boolean): HonestyEar {
  if (!seated) return "unseated";
...
export async function runPorchStt(data: TranscribeInput): Promise<TranscribeResult> {
  ...
  try {
    ...
    return fail("The local word-ear could not be reached.");
  }
